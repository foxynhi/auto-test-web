...
namespace MatKinhShadyTest.Pages.AccountPage
{
  public class AddressPage : BasePage
  {
    ...
    {
      ExtentReporting.LogInfo("Checking if on Address page.");
    ...
    {
      ExtentReporting.LogInfo("Adding new address to user profile.");
      try
      {
      ...
      {
        ExtentReporting.LogInfo($"Failed to add address: {ex.Message}");
        return false;
      }
    }
    public string GetFullName(string address)
    {
      ExtentReporting.LogInfo("Retrieving full name from address entry.");
      return WaitUtility.WaitForElementToBeVisible(ViewFullName(address)).Text;
    }
    public string GetAddress(string address)
    {
      ExtentReporting.LogInfo("Retrieving address from address entry.");
      return WaitUtility.WaitForElementToBeVisible(ViewAddress(address)).Text;
    }
    public string GetPhoneNumber(string address)
    {
      ExtentReporting.LogInfo("Retrieving phone number from address entry.");
